...
//  MyMenu
//
//  Created by Sebastian Connelly (student LM) on 4/30/18.
...
class AllIngredients{
    ...
    init(){
        ...
        categories.append(contentsOf: ["Dairy", "Vegetables & Legumes", "Fruits", "Baking & Grains", "Protein", "Condiments", "Oils, Seasonings, & Salts", "Beverages", "Soup", "Nuts", "Desserts & Snacks"])
    ...
    func generateRandomIngredients(number: Int) -> [String] {
        var ing = [String]()
        
        while ing.count < number {
            var randomNumber: Int = Int(arc4random_uniform(UInt32(ingredients.count)))
            var randomNumber2: Int = Int(arc4random_uniform(UInt32(ingredients[categories[randomNumber]]!.count)))
            let i = ingredients[categories[randomNumber]]![randomNumber2]
            if !ing.contains(i) {
                ing.append(i)
            }
        }
